...
def printPacketType1(serialData, ids):
    strRorg = "RORG: "
    ...
    if strRorg[len(strRorg)-2:len(strRorg)] == 'f6':
        ...
        ()
    print strData + "\n"
    if(serialData[10:18] not in ids):
        ids.append(serialData[10:18])
    return ids
...
def commandReadBaseId(): 
    ...
    if int(returnData[0:1],16) == 0x00:
        print("BaseId: " + str(returnData[2:10]))
        return returnData[2:10]
    else:
        return Exception("Response Error:" + returnData[0])
...
def main(): 
    ...
    print('------------------------------------------')
    ser = serial.Serial(port, 57600, timeout = 0)  # open serial port
    ...
    while True:
        try:
            base_id = commandReadBaseId()
            break
        except Exception as e:
            print(e)
    base_id_string = base_id[0:2]
    for x in range(2,len(base_id),2):
    	base_id_string += ":" + base_id[x:x+2]
    
    #base_uri = raw_input("\nsvc_base_uri (e.g scratch.ns/ligth_test): ")
    #name_serv = raw_input("\nname (e.g kitchen): ")
    #pollInt = raw_input("\npoll_interval (e.g 10s): ")

    response = "Yes"
    pair_word = [165,28,16,27,128,0,0,0,0,128]
    switch_names = []
    switch_modes = []
    i = 0
    while (response[0]=="Y" or response[0]=="y") and i<11:
        print("\nPress the P button on the switch until it starts to blink")
        ready = "No"
        while not (ready[0]=='Y' or ready[0]=='y'):
            ready = raw_input("\nReady? (Yes/No): ")
        # Increment ligth_id
        light_id = hex(int(base_id, 16)+i)[2:]
        if (len(light_id)%2 != 0):
            light_id = '0'+light_id
        pair_word[5:9] = [int(light_id[j:j+2],16) for j in range(0, len(light_id), 2)]
        # Send pairing word
        sendESP3Packet(0x01,pair_word)
        switch_names.append(raw_input("\nName of the switch: "))
        switch_modes.append(raw_input("\nMode of the switch (e.g 15m): "))
        response = raw_input("\nDo you want to continue pairing lights? (Yes/No): ")
        print("----------------------------------------------------------------")
        i = i + 1
    # Write the yaml file
    data = dict(
    	port = port,
        svc_base_uri = 'scratch.ns/ligth_test',
        name = "kitchen",
        poll_interval= "10s",
        switch_mode = switch_modes,
        USB300_id = base_id_string,
